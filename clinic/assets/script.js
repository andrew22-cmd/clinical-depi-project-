...
function goBackFromDoctorsSchedules() {
    ...
    if (role === 'manager') {
        goTo('manager-dashboard.html');
        return;
    }

    if (role === 'secretary') {
        goTo('secretary-dashboard.html');
        return;
    }
...
function handleLogin(event) {
    ...
    if (role === 'manager') return goTo('manager-dashboard.html');
    if (role === 'secretary') return goTo('secretary-dashboard.html');
    if (role === 'doctor') return goTo('doctor-dashboard.html');
...
function addDoctorSchedule(event) {
    ...
    setTimeout(() => goTo('doctors-schedules.html'), 700);
}
